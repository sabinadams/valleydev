...
import { AiFillInfoCircle, AiTwotoneMail } from "react-icons/ai"
...
export function Router() {
    return (
        <div className="fixed z-20 h-full w-1/6 flex items-center group">
            ...
            <div className="w-16 bg-slate-400 p-4 gap-y-4 text-slate-600 text-3xl items-center justify-start shrink-0 h-2/3 tranform transition duration-300 ease-in-out -translate-x-28 rounded-2xl group-hover:translate-x-0 scale-50 group-hover:scale-100 flex flex-col shadow-lg">
                <LinkOption element={FaHome} onClick={() => scrollToNext('home')} />
                <Divider />
                <LinkOption element={FaCode} onClick={() => scrollToNext('tech')} />
                <Divider />
                <LinkOption element={AiFillInfoCircle} onClick={() => scrollToNext('about')} />
                <Divider />
                <LinkOption element={AiTwotoneMail} onClick={() => scrollToNext('contact')} />
